...
class VectorRepository:
    ...
    def _filters(
        ...
        if document_ids:
            ...
            for did in document_ids:
                cond = Filter.by_property("document_id").equal(did)
    ...
    def query_documents_by_vector(
        ...
        *,
        ...
        flt = self._filters(document_type_id=document_type_id)
        resp = self._docs().query.near_vector(
        ...
        for o in objs:
            ...
            distance = float(meta.distance) if (meta is not None and meta.distance is not None) else None
            out.append({
    ...
    def query_chunks_by_vector(
        ...
        *,
        ...
        resp = self._chunks().query.near_vector(
            near_vector=query_vec,
            limit=limit,
            filters=flt,
            return_metadata=MetadataQuery(distance=True),
    ...
    def hybrid_query_chunks(
        ...
        *,
        ...
        resp = self._chunks().query.hybrid(
            query=query_text,
            vector=vector,
            alpha=alpha,
            limit=limit,
