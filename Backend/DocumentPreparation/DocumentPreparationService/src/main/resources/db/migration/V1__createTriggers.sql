DROP TABLE statistika_projektnih_dokumenata CASCADE CONSTRAINTS;
DROP TABLE statistika_projekta CASCADE CONSTRAINTS;
...
CREATE TABLE statistika_projekta (
                                     id integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                                     projekat_id integer,
                                     broj_neispostovanih_rokova integer,
                                     broj_zatrazenih_ispravki integer,
                                     procenat_ispostovanosti_zadatog_roka float
);

CREATE TABLE statistika_projektnih_dokumenata (
                                                  id integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                                                  statistika_projekta_id integer,
                                                  dokument_id integer,
                                                  prekoracenje_roka_u_procentima float
);


ALTER TABLE statistika_projektnih_dokumenata ADD FOREIGN KEY (dokument_id) REFERENCES dokument (id);
ALTER TABLE statistika_projektnih_dokumenata ADD FOREIGN KEY (statistika_projekta_id) REFERENCES statistika_projekta (id);
ALTER TABLE statistika_projekta ADD FOREIGN KEY (projekat_id) REFERENCES projekat (id);
...
ALTER TABLE status_log ADD FOREIGN KEY (prethodno_stanje) REFERENCES status (id);
ALTER TABLE status_log ADD FOREIGN KEY (novo_stanje) REFERENCES status (id);
ALTER TABLE status_log ADD FOREIGN KEY (projekat_id) REFERENCES projekat (id);

CREATE OR REPLACE TRIGGER trg_status_log_datum
